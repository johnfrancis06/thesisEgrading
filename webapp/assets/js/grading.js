...
function showAddItemModal() {
    ...
    // Auto-set label based on existing items
    ...
    itemRows.forEach(row => {
        ...
        if (row.classList.contains('text-center') || row.querySelector('td[colspan="5"]')) {
            return;
        }
        const cells = row.querySelectorAll('td');
        if (cells.length >= 4) {
            const label = cells[1].textContent.trim();
            const sortOrder = parseInt(cells[3].textContent) || 0;
            itemData.push({ label, sortOrder });
        }
    });
    
    if (itemData.length > 0) {
        // Sort by sortOrder to get the last item
        itemData.sort((a, b) => a.sortOrder - b.sortOrder);
        ...
        if (match) {
            ...
            document.getElementById('gradeItemLabel').value = `${prefix}${number + 1}`;
        } else {
            // No trailing number found, leave blank
            document.getElementById('gradeItemLabel').value = '';
        }
    } else {
        // No existing items, leave blank for user to set the pattern
        document.getElementById('gradeItemLabel').value = '';
    }
